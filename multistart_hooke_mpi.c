...
int main(int argc, char *argv[])
{
	...
	if (myid == 0)
                start = 0;
        else if (myid < ntrials % procs)
                start = myid * (ntrials / procs + 1);
        else
                start = ntrials % procs + myid * (ntrials / procs);

        if (myid == procs - 1)
                end = ntrials - 1;
        else if (myid < ntrials % procs)
                end = start + ntrials / procs + 1;
        else
                end = start + ntrials / procs;
	...
	for (trial = start ; trial < end; ++trial) {
		...
		jj = hooke(nvars, startpt, endpt, rho, epsilon, itermax,myid,procs);
#if DEBUG
